...
describe("apple/config", () => {
  ...
  describe("store download endpoints", () => {
    it("volumeStore targets MZFinance with the externalVersionId key", () => {
      const endpoint = volumeStoreEndpoint("42", "aabbccddeeff");

      expect(endpoint.host).toBe("p42-buy.itunes.apple.com");
      expect(endpoint.path).toBe(
        "/WebObjects/MZFinance.woa/wa/volumeStoreDownloadProduct?guid=aabbccddeeff",
      );
      expect(endpoint.externalVersionIdKey).toBe("externalVersionId");
    });

    it("redownload targets downloaddispatch with the appExtVrsId key", () => {
      const endpoint = redownloadEndpoint("aabbccddeeff");

      expect(endpoint.host).toBe("downloaddispatch.itunes.apple.com");
      expect(endpoint.path).toBe("/r/redownload?guid=aabbccddeeff");
      expect(endpoint.externalVersionIdKey).toBe("appExtVrsId");
    });
