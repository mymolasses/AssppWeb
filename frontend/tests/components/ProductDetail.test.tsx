...
import { useUiPreferencesStore } from '../../src/store/uiPreferences';
...
describe('ProductDetail download action', () => {
  beforeEach(() => {
    ...
    useUiPreferencesStore.setState({ selectedAccountEmail: '' });
  });
  ...
  it('is disabled on the initial commit before an account is selected', async () => {
    ...
    renderProductDetail(() => {
      const button =
        document.querySelector<HTMLButtonElement>('button[aria-busy]');
  ...
  it('retains accounts from other regions and remembers an explicit selection', async () => {
    const otherAccount = {
      ...account,
      email: 'uk@example.test',
      store: '143444',
    };
    mocks.accounts = [account, otherAccount];
    renderProductDetail();

    const select = screen.getByRole('combobox');
    expect(
      screen.getByRole('option', { name: /uk@example.test/ }),
    ).toBeInTheDocument();
    fireEvent.change(select, { target: { value: otherAccount.email } });

    expect(select).toHaveValue(otherAccount.email);
    expect(useUiPreferencesStore.getState().selectedAccountEmail).toBe(
      otherAccount.email,
    );
    fireEvent.click(
    ...
    await waitFor(() =>
      expect(mocks.startDownload).toHaveBeenCalledWith(otherAccount, app),
    );
  });
